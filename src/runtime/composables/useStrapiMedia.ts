...
export const useStrapiMedia = () => {
    ...
    const uploadSingleMedia = async (formData?: FormData | null | undefined, fileInfo?: fileInfoProps, isForAdmin = false, onUploadProgress?: (progressEvent: AxiosProgressEvent) => void) => {
        try {
            ...
            if(!formData) throw 'Please select at least one file.';
    ...
    const postFormDataRequest = async (formData:FormData, fileInfo?: fileInfoProps, isForAdmin = false, onUploadProgress?: (progressEvent: AxiosProgressEvent) => void, path?: string) => {
